...
```mermaid
flowchart TD
  A([Start]) --> B{Is the data needed
  for the first render?}

  B -- No --> C[Client-side fetch
  or Suspense]
  B -- Yes --> D{Does SEO/metadata
  depend on this data?}

  D -- Yes --> E[Server-side (metadata/SEO)]
  D -- No --> F{Is the data user-specific?}

  F -- No --> G{Is the data static
  or semi-static?}
  F -- Yes --> H{How slow is the request
  and how big is the payload?}

  G -- Yes --> I[SSG or ISR]
  G -- No --> H

  H -- "< 300ms and < 150kb" --> J[Blocking server fetch is OK]
  H -- "300-700ms or 50-300kb" --> K[Server fetch + Suspense
  (streaming)]
  H -- "> 700ms or > 300kb" --> L[Suspense or
  client-side]

  I --> M{Must the data remain
